...
# DeepBrain GraphRag - Forked from original LightRag project from HKUDS
...
1. **Clone the `lightrag` repository**  
   ```bash
   git clone https://code.thinkdeep.ai/deepbrain/lightrag
   ```

2. **Configure environment variables:**  
   - Add the `.env` file in the `lightrag` folder (provide a sample or structure if needed).
   - Add the `GRAPHRAG` configuration in the `.env` file of `deepBrain` (specify the key details if necessary).
...
Run the Ollama container on the Docker network used by DeepBrain:

```bash
docker run -d   --name ollama   --network deepbrain_network   -v ollama:/root/.ollama   -p 11434:11434   ollama/ollama
```

#### If you need to restart or fix Ollama:
...
From the DeepBrain project folder:
...
From the Lightrag project folder:
...
## Accessing the Lightrag Interface

Once all services are up and running, you can access the Lightrag web interface at:

[http://localhost:9621/webui/#/](http://localhost:9621/webui/#/)
...
- **LLM Selection**:
  ...
  - **Structured Output (OpenAI/Azure)**: If using OpenAI or Azure OpenAI, you can enable `use_structured_output_for_extraction=True` to eliminate entity extraction parsing errors caused by malformed LLM responses. This uses JSON schema validation to ensure consistent output format.
- **Embedding Model**:
...
| **use_structured_output_for_extraction** | `bool` | If `TRUE`, uses JSON schema structured output for entity extraction (OpenAI/Azure only). Eliminates parsing errors from malformed LLM responses. | `FALSE` |
