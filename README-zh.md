...
- **LLM选型**：
  ...
  - **结构化输出（OpenAI/Azure）**：如果使用OpenAI或Azure OpenAI，可以启用`use_structured_output_for_extraction=True`来消除因LLM响应格式错误导致的实体提取解析错误。这使用JSON模式验证来确保输出格式一致。
- **Embedding模型**：
...
| **use_structured_output_for_extraction** | `bool` | 如果为`TRUE`，使用JSON结构化输出进行实体提取（仅限OpenAI/Azure）。消除因LLM响应格式错误导致的解析错误。 | `FALSE` |
